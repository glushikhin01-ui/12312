...
function meta:PickupItem( ent )
	...
	if slot then
		-- hook.Run( "ItemStoreOnPickup", self, ent )
...
function meta:SplitItem( con_id, slot, amount )
	...
	if con and con:FirstEmptySlot() and item and item:CanSplit( amount ) and
...
local cooldownUp = 0.5

hook.Add( "PlayerUse", "ItemStorePickup", function( pl, ent )
	if not IsValid( pl ) or not pl:IsPlayer() then return end
	if not pl.LastActionTime then pl.LastActionTime = 0 end

	if CurTime() - pl.LastActionTime >= cooldownUp then
