...
function CH_CryptoCurrencies.CryptoMenu()
	...
	GUI_CryptoFrame.Think = function()
        if input.IsKeyDown(KEY_ESCAPE) then
            GUI_CryptoFrame:AlphaTo(0, 0.2, 0,function()
	...
	if CH_CryptoCurrencies.Config.EnableSQL then
		...
		GUI_TransactionsFrameBtn.Paint = function( self, w, h )
			...
			text( CH_CryptoCurrencies.LangString( "Transactions" ), "MSB_20", w / 2, h / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
		end
	...
	panel_info.Paint = function(s, w, h)
		...
		text('Купить/Продать', 'MM_18', (w - ss(65)), h * .5, colors['white_25'], TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
	...
	GUI_CryptoList.Paint = function( self, w, h )
		box( 0, 0, 0, w, h, CH_CryptoCurrencies.Colors.Invisible )
	end
	...
	for index, crypto in ipairs( CH_CryptoCurrencies.CryptosCL ) do
		if crypto.Name then
			...
			if price_change < 0 then
				price_change_color = colors['red']
			end
			local no_change = false
			
			-- Panel per crypto
			local GUI_CryptoPanel = vgui.Create( "DPanel", GUI_CryptoList )
			GUI_CryptoPanel:Dock(TOP)
			GUI_CryptoPanel:DockMargin(0, 0, ss(10), ss(10))
			GUI_CryptoPanel:SetTall(ss(82))
			GUI_CryptoPanel.Paint = function( self, w, h )
				-- Background
				box( 8, 0, 0, w - ss(184), h, CH_CryptoCurrencies.Colors.DarkGray )
				...
				if not no_change then
					text( price_change .."%", "MM_18", w - ss(198), ss(62), price_change_color, TEXT_ALIGN_RIGHT, TEXT_ALIGN_BOTTOM )
				end
			...
			GUI_SellCrypto:Dock(RIGHT)
			GUI_SellCrypto:SetWide(ss(82))
			GUI_SellCrypto:SetTextColor( Color( 0, 0, 0, 255 ) )
			GUI_SellCrypto:SetText( "" )
			GUI_SellCrypto.Paint = function( s, w, h )
				box(10, 0, 0, w, h, CH_CryptoCurrencies.Colors.DarkGray)
				setmat(materials['no'])
				setcolor(s:IsHovered() and player_owns > 0 and color_white or colors['white_25'])
				setsize(ss(9), ss(9), ss(64), ss(64))
			end
			...
			GUI_BuyCrypto:Dock(RIGHT)
			GUI_BuyCrypto:DockMargin(0, 0, ss(10), 0)
			GUI_BuyCrypto:SetWide(ss(82))
			GUI_BuyCrypto:SetTextColor( Color( 0, 0, 0, 255 ) )
			GUI_BuyCrypto:SetText( "" )
			GUI_BuyCrypto.Paint = function( s, w, h )
				box(10, 0, 0, w, h, CH_CryptoCurrencies.Colors.DarkGray)
				setmat(materials['yes'])
				setcolor(s:IsHovered() and color_white or colors['white_25'])
				setsize(ss(9), ss(9), ss(64), ss(64))
			end
