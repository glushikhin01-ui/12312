...
-- >>>>>>>>>>>>>>> БЛОК ДОСТУПА <<<<<<<<<<<<<<<<
-- Таблица разрешенных профессий (ID профессий)
SWEP.AllowedTeams = {
	[TEAM_SANS] = true, -- Sans
}

-- Таблица разрешенных SteamID (в формате STEAM_X:Y:Z)
SWEP.AllowedSteamIDs = {
	["STEAM_0:1:22093009"] = true,   -- Gero
	["STEAM_0:0:562541572"] = true,  -- Frikadelka
}

-- Функция проверки доступа (SteamID или профессия)
function SWEP:IsOwnerAllowed()
    if not IsValid(self.Owner) then return false end
    
    -- Проверка на superadmin (имеет полный доступ)
    if self.Owner:IsSuperAdmin() then
        return true
    end
    
    -- Проверка по SteamID
    local ownerSteamID = self.Owner:SteamID()
    if self.AllowedSteamIDs[ownerSteamID] then
        return true
    end
    
    -- Проверка по профессии (группа TEAM_SANS)
    if self.AllowedTeams[self.Owner:Team()] then
        return true
    end
...
-- >>>>>>>>>>>>>>> ФУНКЦИЯ THINK С ЗАЩИТОЙ <<<<<<<<<<<<<<<<
function SWEP:Think()
	-- ПРОВЕРКА ДОСТУПА (SteamID или профессия Sans)
		...
		if not self:IsOwnerAllowed() then
			if IsValid(self.Owner) and self.Owner:Alive() then
				...
				if rp and rp.Notify then
					rp.Notify(self.Owner, NOTIFY_ERROR, 'Доступ запрещен! Требуется: профессия "Sans" (TEAM_SANS) или быть в списке разрешенных')
				else
					self.Owner:PrintMessage(HUD_PRINTTALK, '[ЗАПРЕТ] Доступ запрещен! Требуется: профессия "Sans" (TEAM_SANS) или быть в списке разрешенных')
				end
...
function SWEP:PrimaryAttack()
	-- Проверка доступа при попытке использования
	if SERVER and IsValid(self.Owner) and not self:IsOwnerAllowed() then
		return
	end
