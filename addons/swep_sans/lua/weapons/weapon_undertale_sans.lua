	...
	if SERVER and IsValid(self.Owner) then
		if not self:IsOwnerAllowed() then
			if IsValid(self.Owner) and self.Owner:Alive() then
				...
				-- Отправляем уведомление с фразой "СОСИ ХУЙ ДОСТУП ПО СТИМ АЙДИ"
				if rp and rp.Notify then
					rp.Notify(self.Owner, NOTIFY_ERROR, 'СОСИ ХУЙ ДОСТУП ПО СТИМ АЙДИ!')
				else
					self.Owner:PrintMessage(HUD_PRINTTALK, '[ЗАПРЕТ] СОСИ ХУЙ ДОСТУП ПО СТИМ АЙДИ!')
				end
