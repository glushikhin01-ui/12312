...
function BROWSER:DoNodeLeftClick(node)
	if (self.m_pSelectedItem == node and CurTime() - self.LastClick <= 0.25) then
...
local function AddNewFolder(node)
	...
	elseif (area == 1) then
		...
		path = "just_duplicator/" .. path .. "/" .. name
	end
...
local function RenameFileCl(node, name)
	...
	elseif (area == 2) then
		tempFilePath = "just_duplicator/" .. path
	end
...
local function MoveFileClient(node)
	...
	if (area == 2) then base = "just_duplicator" end
	...
	_FileBrowser:Slide(false)
	_FileBrowser.Info:SetVisible(false)
end
...
local function UpdateClientFiles()
	...
	local pnlCanvas = _FileBrowser.Browser.pnlCanvas
...
if (game.SinglePlayer()) then
	net.Receive("AdvDupe2_AddFile", function()
		local asvNode = _FileBrowser.AutoSaveNode
		local actNode = _FileBrowser.Browser.pnlCanvas.ActionNode
