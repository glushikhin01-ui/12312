local function SaveFile(ply, cmd, args)
	...
	net.Start("AdvDupe2_SetDupeInfo")
		...
		net.WriteString(tostring(table.Count(ply.AdvDupe2.Entities)))
		net.WriteString(tostring(#ply.AdvDupe2.Constraints))
	net.Send(ply)
...
function AdvDupe2.SendToClient(ply, data, autosave)
	...
	local stream = net.WriteStream(data, function()
		if IsValid(ply) and ply.AdvDupe2 then
			ply.AdvDupe2.Downloading = false
			AdvDupe2.RemoveProgressBar(ply)
			timer.Remove("AdvDupe2_DownloadSafety_" .. ply:SteamID64())
		end
	end)
	net.Send(ply)
	...
	if not stream then
		ply.AdvDupe2.Downloading = false
		AdvDupe2.RemoveProgressBar(ply)
		AdvDupe2.Notify(ply, "File was not saved!", NOTIFY_ERROR)
		return
	end

	timer.Create("AdvDupe2_DownloadSafety_" .. ply:SteamID64(), 120, 1, function()
		if IsValid(ply) and ply.AdvDupe2 and ply.AdvDupe2.Downloading then
			ply.AdvDupe2.Downloading = false
			AdvDupe2.RemoveProgressBar(ply)
		end
	end)
