local function AdvDupe2_ReceiveFile(len, ply)
	...
	net.ReadStream(nil, function(data)
		...
		local fb = AdvDupe2.FileBrowser
		if(AutoSave)then
			if(fb and IsValid(fb.AutoSaveNode))then
				local add = true
				for i=1, #fb.AutoSaveNode.Files do
					if(filename==fb.AutoSaveNode.Files[i].Label:GetText())then
						add=false
						break
					end
				end
				if(add)then
					fb.AutoSaveNode:AddFile(filename)
					fb.Browser.pnlCanvas:Sort(fb.AutoSaveNode)
				end
			end
		elseif(fb and fb.Browser)then
			fb.Browser.pnlCanvas.ActionNode:AddFile(filename)
			fb.Browser.pnlCanvas:Sort(fb.Browser.pnlCanvas.ActionNode)
		end
		if(!errored)then
