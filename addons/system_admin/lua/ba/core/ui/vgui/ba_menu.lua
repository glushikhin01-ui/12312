...
function PANEL:Init()
	...
	self.Copy:SetText('скопирован SteamID')
...
function PANEL:SetPlayer(pl)
	if !IsValid(pl) then self:Remove() return end
	self.Player = pl
...
function PANEL:Init()
	self.lblTitle:SetText('Настройки')
	self.Settings = ui.Create('DButton', self)
	self.Settings:SetText('')
	surface.SetFont('ui.22')
	self.Settings:SetSize(surface.GetTextSize('Настройки') + 10, 27)
...
function PANEL:Init()
	self.w, self.h = ScrW() * .25, ScrH() * .425
	self.h3 = self.h * 1.5
	
	self.lblTitle:SetText('Админ меню')
