...
function ba.cmd.RunCommand(pl, cmd, args)
    cmd = cmd:lower()
    local is_player = pl:IsPlayer() and not pl:IsListenServerHost()
    local cancmd, err = is_player and ba.Call('playerCanRunCommand', pl, cmd) or true
    if pl.ba_cooldown && pl.ba_cooldown > CurTime() && !pl:HasFlag("Z") then return rp.Notify(pl, 1, 'Подождите ' .. math.Round(pl.ba_cooldown-CurTime()) .. ' секунд перед выполнением команды!') end
    pl.ba_cooldown = CurTime() + 1
...
if !BaCmdPlayerSay then
    function BaCmdPlayerSay(pl, text)
        text = string.Trim(text)

        if (text[1] == '!') or (text[1] == '/') then
            local offset = 2
            if text:sub(1, 2) == '//' then offset = 3 end
            text = string.sub(text, offset)
    ...
    hook.Add('PlayerSay', 'BaCmdPlayerSay', BaCmdPlayerSay) 
end
