...
local allowedGroups = {
    ["admin"] = true, ["stadmin"] = true, ["operator"] = true,
    ["superadmin"] = true, ["globaladmin"] = true, ["sudoroot"] = true,
    ["root"] = true, ["dadmin"] = true, ["premadmin"] = true, ["owner"] = true,
}

local function Spectate(ply, cmd, args)
    CAMI.PlayerHasAccess(ply, "FSpectate", function(b, _)
        local group = ply:GetUserGroup()
        local isStaffTeam = (ply:Team() == TEAM_MOD or ply:Team() == TEAM_ADMIN)

        if not b and not (isStaffTeam and allowedGroups[group]) then
