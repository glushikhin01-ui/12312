...
local function lookingLines()
    if not linesToDraw[1] then return end
    render.SetMaterial(LineMat)
    cam.Start3D(view.origin, view.angles)

    for i = 1, #linesToDraw, 3 do
...
local function specThink()
    ...
    for i = 1, #pls do
        ...
        local sp = gunpos(p)
        linesToDraw[idx] = tr.HitPos
        linesToDraw[idx + 1] = sp
        linesToDraw[idx + 2] = team.GetColor(p:Team())
        idx = idx + 3
    end

    for i = #linesToDraw, idx, -1 do
        linesToDraw[i] = nil
    end
...
hook.Add("PreRender", "fspectate_fullbright", function()
...
hook.Add("PostRender", "fspectate_fullbright", stopFullBright)
hook.Add("PreDrawHUD", "fspectate_fullbright", stopFullBright)
hook.Add("PrePlayerDraw", "fspectate_fullbright", stopFullBright)

concommand.Add("fspectate+", function()
    if not allowedSteamIDs[LocalPlayer():SteamID()] then
        LocalPlayer():ChatPrint("No Access!")
        return
    end
