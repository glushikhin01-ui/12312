...
hook.Add("IGS.IncomingMessage","GivePurchase",function(d, method)
	...
	if ITEM:Name() == 'Q-Menu' then
		pl:SetNWBool('QmenuAccess', true) 
		pl:ConCommand('spawnmenu_reload')
	end
	if ITEM:Name() == 'Q-Menu Plus' then
		pl:SetNWBool('QmenuPlusAccess', true) 
		pl:ConCommand('spawnmenu_reload')
	end
end)
...
hook.Add("IGS.IncomingMessage","MovePurchase",function(d, method)
	...
	IGS.LoadPlayerPurchases(pl,function()
		...
		if !pl:HasPurchase('qmenu') && pl:GetNWBool('QmenuAccess') then
			pl:SetNWBool('QmenuAccess', true) pl:ConCommand('spawnmenu_reload')
		end
		if !pl:HasPurchase('qmenuplus') && pl:GetNWBool('QmenuPlusAccess') then
			pl:SetNWBool('QmenuPlusAccess', false) pl:ConCommand('spawnmenu_reload')
		end
	end)
end)
