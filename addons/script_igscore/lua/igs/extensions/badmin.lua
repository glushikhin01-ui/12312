...
function STORE_ITEM:SetBAdminGroup(rank)
	return self:SetInstaller(function(pl)
		...
		local current = ba.ranks.Get(pl:GetNetVar("UserGroup"));
		if (current && current:GetImmunity() > RANK:GetImmunity()) then return end;
		...
		-- print("On Purchare", pl:GetNetVar("UserGroup"))
