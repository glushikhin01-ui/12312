...
util.AddNetworkString("IGS.GiveOnSpawnWep")
    ...
    hook.Add("IGS.PlayerActivatedItem","IGS.PlayerLoadout",function(pl, ITEM)
		...
		if ITEM.uid == 'props' then
			local purchs = pl:HasPurchase("props")
			timer.Simple(1, function () pl:SetNW2Int('PurchasedProps', purchs * 50) end)
		end

    	if ITEM.uid == 'qmenu' then pl:SetNWBool('QmenuAccess', true) end
    	if ITEM.uid == 'qmenuplus' then pl:SetNWBool('QmenuPlusAccess', true) end
    	if ITEM.uid == 'sopmod_job' then pl:SetNWBool('SopmodAccess', true) end
    	if ITEM.uid == 'kaine_job' then pl:SetNWBool('KaineAccess', true) end
    	if ITEM.uid == 'shtirlec_job' then pl:SetNWBool('ShtirlicAccess', true) end
    	if ITEM.uid == 'cannibal_job' then pl:SetNWBool('CannibalAccess', true) end
    	if ITEM.uid == 'nier_job' then pl:SetNWBool('NierAccess', true) end
    	if ITEM.uid == 'berserk_job' then pl:SetNWBool('BerserkAccess', true) end
    	if ITEM.uid == 'bulldozer_job' then pl:SetNWBool('BdozerAccess', true) end
    	if ITEM.uid == 'elza_job' then pl:SetNWBool('ElzaAccess', true) end
    	if ITEM.uid == 'muffler_job' then pl:SetNWBool('MufflerAccess', true) end
    	if ITEM.uid == 'haku_job' then pl:SetNWBool('HakuAccess', true) end
    	if ITEM.uid == 'roxy_job' then pl:SetNWBool('RoxyAccess', true) end
    	if ITEM.uid == 'sans_job' then pl:SetNWBool('SansAccess', true) end
    	if ITEM.uid == 'mag_akolit' then pl:SetNWBool('AkolitAccess', true) end
    	if ITEM.uid == 'mag_koldun' then pl:SetNWBool('KoldunAccess', true) end
    	if ITEM.uid == 'mag_charodei' then pl:SetNWBool('CharodeiAccess', true) end
    	if ITEM.uid == 'mag_chernoknizhnik' then pl:SetNWBool('ChernoknigAccess', true) end
    	if ITEM.uid == 'mag_archimag' then pl:SetNWBool('ArchimagAccess', true) end
    	if ITEM.uid == 'shadow_job' then pl:SetNWBool('ShadowAccess', true) end
    	if ITEM.uid == 'sinon_job' then pl:SetNWBool('SinonAccess', true) end
    	if ITEM.uid == 'segun_job' then pl:SetNWBool('SegunAccess', true) end
    	if ITEM.uid == 'hranitel_job' then pl:SetNWBool('HranitelAccess', true) end
    	if ITEM.uid == 'jiangyu_job' then pl:SetNWBool('JiangyuAccess', true) end
    	if ITEM.uid == 'ratigang_job' then pl:SetNWBool('RatigangAccess', true) end
...
-- =========================
-- IGS FIX: выдача донат оружия при смене профессии
-- =========================

if SERVER then

    local function GiveDonateWeapons(ply)
        if not IsValid(ply) then return end
        hook.Call("IGS_PlayerLoadout", IGS, ply)
    end

    -- смена профессии (ГЛАВНЫЙ ФИКС)
    hook.Add("OnPlayerChangedTeam", "IGS_Fix_JobWeapons", function(ply)
        timer.Simple(0.2, function()
            GiveDonateWeapons(ply)
        end)
    end)

    -- подстраховка (если кто-то чистит оружие)
    hook.Add("PlayerSpawn", "IGS_Fix_SpawnWeapons", function(ply)
        timer.Simple(0.2, function()
            GiveDonateWeapons(ply)
        end)
    end)
