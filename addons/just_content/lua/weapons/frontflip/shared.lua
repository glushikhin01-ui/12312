...
SWEP.SlotPos = 1
SWEP.DrawAmmo = false
SWEP.Instructions = "Hold left click to make a flip and right click to say EXPLOSION?"
SWEP.Contact = "Leafdroids@gmail.com"
SWEP.Purpose = "Making flips"
...
ply:SetNWFloat("ang1", Lerp(FT*6, ang1, 1) )
ply:SetNWFloat("ang2", Lerp(FT*6, ang1, 1) )
else
...
ply:SetNWFloat("ang2", Lerp(FT*200, ang2, 0) )
	...
	if IsValid(ply) and SERVER then
		...
		local bone = ply:LookupBone("ValveBiped.Bip01_Spine1")
		if bone then 
		ply:ManipulateBoneAngles( bone, Angle(0*ang1,360*ang1,0*ang1) )
		end
		
		local bone = ply:LookupBone("ValveBiped.Bip01_L_Thigh")
		if bone then 
		ply:ManipulateBoneAngles( bone, Angle(0*ang1,360*ang1,0*ang1) )
		end
		
		local bone = ply:LookupBone("ValveBiped.Bip01_R_Thigh")
		if bone then 
		ply:ManipulateBoneAngles( bone, Angle(0*ang1,360*ang1,0*ang1) )
		end
...
self.Weapon:EmitSound("physics/cardboard/cardboard_box_break1.wav", 75, 100)
end
