...
sReward.openRewards = function(x, y)
    ...
    if sReward.config["enabled_tabs"]["tasks"] then
        sreward_menu:addTab('Задания', "sreward/tabs/tasks.png")
    end
    ...
    if IsValid(sreward_menu.tab['Задания']) then
        local taskscanvas = vgui.Create("SScrollPanel", sreward_menu.tab['Задания'])
        ...
        for k,v in ipairs(sReward.config["rewards"]) do
            ...
            local verify = task.addButton('Проверить', function()
