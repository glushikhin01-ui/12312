...
function ENT:PlaceBet(ply, pad)
	...
	eui.battlepass.AddProgress(ply, 5)
	...
	for k, v in pairs(self.curHands) do
		for n, m in ipairs(v) do
			...
			elseif handValue > dealerValue then -- You beat the house!
				PerfectCasino.Core.Msg(string.format(PerfectCasino.Translation.Chat.HandWin, PerfectCasino.Config.FormatMoney(m.bet*self.data.payout.win)), m.ply)
				PerfectCasino.Config.AddMoney(m.ply, m.bet*self.data.payout.win)
				hook.Run("pCasinoOnBlackjackPayout", m.ply, self, m.bet*self.data.payout.win)
			end
		end
