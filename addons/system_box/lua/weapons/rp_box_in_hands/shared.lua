...
if CLIENT then
    SWEP.PrintName = "Коробка Грузчика"
    SWEP.Slot = 1
    SWEP.SlotPos = 5
    SWEP.DrawAmmo = false
...
function SWEP:Holster()
    ...
    timer.Simple( 0.01, function()
        ...
        if ply:GetNWBool("TakeBox", false) == true then
			...
			ply:SetCanWalk( true )
			ply:SendMessageFD(Color(0,255,128), "[Грузчик]", Color(255,255,255), "Ты сломал коробку! Не пытайся использовать оружия с ней!")
			ply:StripWeapon("box_in_hands")
		end
    end )
