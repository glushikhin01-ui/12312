...
function ENT:Draw()
	...
	cam.Start3D2D( self:GetPos() + self:GetUp() * 25, Angle( 0, AngEyes.y - 90, 90 ), 0.12 )
		...
		draw.SimpleText( 'Тележка', "MSB_30", -120, 80, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER )
...
hook.Add("HUDPaint","PaintRpBoxHud", function()
	if LocalPlayer():GetNWBool("TakeBox", false) and LocalPlayer():IsArrested() then
		for k,v in pairs(ents.FindByClass("npc_ashot")) do
			DrawPosInfoFD("icon16/box.png",v:EyePos(), 'Тележка')
		end
	end
end)
