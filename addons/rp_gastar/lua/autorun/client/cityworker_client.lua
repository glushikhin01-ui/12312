...
net.Receive( "CITYWORKER.StartAction", function()
    ...
    hook.Add( "HUDPaint", "CITYWORKER.Action.HUDPaint", function()
        ...
        draw.SimpleText( desc, "ui.24", x + ( w / 2 ), y + ( h / 2 ) - 10, Color( 255, 255, 255 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
        draw.SimpleText( CITYWORKER.Config.Language["CANCEL"], "ui.18", x + ( w / 2 ), y + ( h / 2 ) + 12, Color( 255, 255, 255 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
    end )
...
net.Receive( "CITYWORKER.NotifyTask", function()
    ...
    hook.Add( "HUDPaint", "CITYWORKER.Task.HUDPaint", function()
        ...
        draw.SimpleTextOutlined( math.ceil( ( LocalPlayer():GetPos():Distance( pos ) / 16 ) / 3.28084 ).."m", "ui.24", screenPos.x, screenPos.y + 16, Color( 255, 255, 255 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 1, Color( 0, 0, 0 ) )
    end )
end )
