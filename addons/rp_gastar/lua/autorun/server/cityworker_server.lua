...
CITYWORKER_DATA = CITYWORKER_DATA or {}
local availableJobs = {}
local fireHydrants = {}

function CITYWORKER.Begin( ply, ent )
    if ply:Team() != TEAM_WORKER then
        DarkRP.notify( ply, 1, 4, CITYWORKER.Config.Language["NOT_CITY_WORKER"] )
        return
    end

    if ent.CW_isWorked then
        DarkRP.notify( ply, 1, 4, CITYWORKER.Config.Language["JOB_WORKED"] )
        return
    end

    if ply.CW_ent != ent then
        DarkRP.notify( ply, 1, 4, CITYWORKER.Config.Language["ASSIGNED_ELSE"] )
        return
    end
...
function CITYWORKER.Finish( ply, ent )
    ...
    ply:addMoney( pay, 'Выполнение работы CITYWORKER' )
    ...
    DarkRP.notify( ply, NOTIFY_SUCCESS, 4, str )

    KVEST.Progress(ply, "gastorbite", 1)
end
...
function CITYWORKER.Cancel( ply, ent )
    ...
    DarkRP.notify( ply, NOTIFY_HINT, 4, CITYWORKER.Config.Language["CANCELLED"] )
end
...
function CITYWORKER.Assign( ply, job, id )
    ...
    DarkRP.notify( ply, 0, 4, CITYWORKER.Config.Language["NEW_JOB"] )
end
...
function CITYWORKER.Initialize()
    ...
    for k, v in pairs( CITYWORKER_DATA[game.GetMap()] ) do
        ...
        -- Using this, we get approximately 76561198045814018 results.
    ...
    timer.Create( "CITYWORKER.Timer", CITYWORKER.Config.Time, 0, function()
        for _, ply in pairs( team.GetPlayers( TEAM_WORKER ) ) do
...
function CITYWORKER.SendData( ply )
    ...
    if not ply:IsSuperAdmin() then
        notification.AddLegacy( "You must be a superadmin to use the configurator tool!", NOTIFY_ERROR, 4 )
...
net.Receive( "CITYWORKER.Add", function( len, ply )
    ...
    if #CITYWORKER_DATA[game.GetMap()] > 255 then 
        DarkRP.notify( ply, 1, 4, "The maximum number of city worker positions is 255!" )
        return
    end
    ...
    DarkRP.notify( ply, 0, 4, "Successfully added new city worker task!" )
end )
...
net.Receive( "CITYWORKER.Remove", function( len, ply )
    ...
    DarkRP.notify( ply, 0, 4, "Successfully removed a city worker task!" )
end )
