...
local V = {
	...
	KeyValues = {
		vehiclescript = "scripts/vehicles/crsk_autos/crsk_tesla_model_x_2015.txt"
	}
}
list.Set( "Vehicles", "crsk_tesla_model_x_2015", V )

if !crsk_model_x_AddProps_List then crsk_model_x_AddProps_List = {} end
hook.Add("PlayerSpawnedVehicle","crsk_model_x_AddProps", function( ply, vehicle )
    if vehicle:GetModel( ) == "models/crsk_autos/tesla/model_x_2015.mdl" then
        // Create the actual prop
        local prop1 = ents.Create( "prop_dynamic_ornament" )
        if ( !IsValid( prop1 ) ) then return end
        prop1:SetModel( "models/crsk_autos/tesla/model_x_2015_extra.mdl" )
        ...
        table.insert(crsk_model_x_AddProps_List, {vehicle, prop1})
...
hook.Add("Think", "crsk_model_x_SyncChanges", function()
	if crsk_model_x_AddProps_List and (!crsk_model_x_SyncChanges_Timer or CurTime() >= crsk_model_x_SyncChanges_Timer) then
		for k, v in pairs(crsk_model_x_AddProps_List) do
			if IsValid(v[2]) then
				...
				crsk_model_x_AddProps_List[k] = nil
			end
		end
		crsk_model_x_SyncChanges_Timer = CurTime()+0.5
	end
end)
