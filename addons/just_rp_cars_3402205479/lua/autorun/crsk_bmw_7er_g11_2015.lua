...
local V = {
	...
	KeyValues = {
		vehiclescript = "scripts/vehicles/crsk_autos/crsk_bmw_7er_g11_2015.txt"
	}
}
list.Set( "Vehicles", "crsk_bmw_7er_g11_2015", V )

if !crsk_7er_g11_AddProps_List then crsk_7er_g11_AddProps_List = {} end
hook.Add("PlayerSpawnedVehicle","crsk_7er_g11_AddProps", function( ply, vehicle )
    if vehicle:GetModel( ) == "models/crsk_autos/bmw/7er_g11_2015.mdl" then
        // Create the actual prop
        local prop1 = ents.Create( "prop_dynamic_ornament" )
        if ( !IsValid( prop1 ) ) then return end
        prop1:SetModel( "models/crsk_autos/bmw/7er_g11_2015_extra.mdl" )
        ...
        table.insert(crsk_7er_g11_AddProps_List, {vehicle, prop1})
...
hook.Add("Think", "crsk_7er_g11_SyncChanges", function()
	if crsk_7er_g11_AddProps_List and (!crsk_7er_g11_SyncChanges_Timer or CurTime() >= crsk_7er_g11_SyncChanges_Timer) then
		for k, v in pairs(crsk_7er_g11_AddProps_List) do
			if IsValid(v[2]) then
				...
				crsk_7er_g11_AddProps_List[k] = nil
			end
		end
		crsk_7er_g11_SyncChanges_Timer = CurTime()+0.5
	end
end)
