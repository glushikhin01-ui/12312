...
do
    ...
    local tbl = eui.materials

    function eui.AddMaterial(path, icon)
        local pathName = string.Explode('/',  path)
        pathName = pathName[3]

        tbl[pathName] = tbl[pathName] or {}
        if tbl[pathName][icon] then return end

		print(path .. icon .. '.png')
        tbl[pathName][icon] = Material(path .. icon .. '.png')
    end

	local err = Material('icon16/user.png')
    function eui.Material(path, icon)
        if not icon then
            icon = path
            path = 'default'
        end

        return tbl[path] and tbl[path][icon] or err
    end
end
