...
if SERVER then
	function ply:setDarkRPVar(sd,va)
		if sd == "channelID" then
			self:SetNWFloat("channelID",va)
		end
		if sd == "channelName" then
			self:SetNWString("channelName",va)
		end
	end
	function ply:addMoney(sd, description)
		self:AddMoney(sd, description)
	end
