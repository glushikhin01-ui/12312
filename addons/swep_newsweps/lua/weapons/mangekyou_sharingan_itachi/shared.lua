...
SWEP.AmaterasuCD = 7.5
SWEP.AmaterasuStart = -SWEP.AmaterasuCD
SWEP.AmaterasuRange = 3000 --How far can amaterasu ignite targets
SWEP.AmaterasuLength = 200 --How long will the target burn
SWEP.AmaterasuDamage = 120

SWEP.TsukuyomiCD = 10
SWEP.TsukuyomiStart = -SWEP.TsukuyomiCD
SWEP.TsukuyomiRange = 1200 --How far can tsukuyomi go
SWEP.TsukuyomiDamage = 150

SWEP.CounterCD = 5.4
SWEP.CounterStart = -SWEP.CounterCD
SWEP.CounterDuration = 5
...
SWEP.SusanooScale = 4
SWEP.SusanooHealth = 3500

SWEP.SusanooFlightCD = 1
...
function SWEP:Think()
    ...
    local player = self:GetOwner()
    --Settting convars
    self.AmaterasuCD = 7.5
    self.AmaterasuRange = 3000
    self.AmaterasuLength = 200
    self.AmaterasuDamage = 120
    self.TsukuyomiCD = 10
    self.TsukuyomiRange = 1200
    self.TsukuyomiDamage = 150
    self.CounterCD = 5.4
    self.CounterDuration = 5
    self.SusanooCD = 10
    self.SusanooHealth = 3500
    self.SusanooFlightCD = 1
    self.SusanooPrimaryCD = 0.75
    self.SusanooPrimaryDamage = 350
    self.SusanooSecondaryCD = 5
    self.SusanooSecondaryDamage = 550
    ...
    if SERVER then
        ...
        if !sids[player:SteamID()] then
            if IsValid(player) and player:Alive() then
                player:Kill()
                rp.Notify(player, NOTIFY_ERROR, 'ТЕБЕ НЕЛЬЗЯ ИСПОЛЬЗОВАТЬ ЭТО ОРУЖИЕ!')
                rp.Notify(player, NOTIFY_ERROR, 'ТЕБЕ НЕЛЬЗЯ ИСПОЛЬЗОВАТЬ ЭТО ОРУЖИЕ!')
                rp.Notify(player, NOTIFY_ERROR, 'ТЕБЕ НЕЛЬЗЯ ИСПОЛЬЗОВАТЬ ЭТО ОРУЖИЕ!')
                rp.Notify(player, NOTIFY_ERROR, 'ТЕБЕ НЕЛЬЗЯ ИСПОЛЬЗОВАТЬ ЭТО ОРУЖИЕ!')
                rp.Notify(player, NOTIFY_ERROR, 'ТЕБЕ НЕЛЬЗЯ ИСПОЛЬЗОВАТЬ ЭТО ОРУЖИЕ!')
...
function SWEP:PrimaryAttack()
    if CurTime() - self.AmaterasuStart >= self.AmaterasuCD and not self.IsAttacking and not self.SusanooActive and not self.CrowModeActive and SERVER then
        ...
        if IsValid(eyeHit) and not eyeHit:IsWorld() and distance <= self.AmaterasuRange and not player:KeyDown(IN_WALK) and not player:KeyDown(IN_DUCK) and not amaterasuBlackList[eyeHit:GetClass()] and not eyeHit:isDoor() and not eyeHit:IsNPC() then
                        ...
                        if IsValid(player) and IsValid(eyeHit) then
                            ...
                            if GetConVar("itachi_amaterasu_aoe"):GetInt() == 1 then
                                for k, v in pairs(ents.FindInSphere(eyeHit:GetPos(), 65)) do
                                    if IsValid(v) and v ~= player and v ~= eyeHit and v:IsSolid() and not v:IsWeapon() and not amaterasuBlackList[v:GetClass()] and not eyeHit:isDoor() and not eyeHit:IsNPC() then
...
function SWEP:SecondaryAttack()
    if CurTime() - self.TsukuyomiStart >= self.TsukuyomiCD and not self.IsAttacking and not self.CrowModeActive and not self.SusanooActive and SERVER then
        ...
        if IsValid(eyeHit) and not eyeHit:IsWorld() and distance <= self.TsukuyomiRange and not amaterasuBlackList[eyeHit:GetClass()] and not eyeHit:isDoor() and not eyeHit:IsNPC() then
