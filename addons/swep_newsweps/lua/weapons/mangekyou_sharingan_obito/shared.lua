...
function SWEP:PrimaryAttack()
    if SERVER then
        if CurTime() - self.KamuiEntStart >= self.KamuiEntCD and not self.IsAttacking and not self.SusanooActive then
            ...
            if traceEnt:IsValid() and not traceEnt:IsWorld() and not kamuiBlackList[traceEnt:GetClass()] and traceEnt:IsPlayer() then
                if distance <= 1000 and not player:KeyDown(IN_WALK) then
                    ...
                    local tracePos = Vector(-1528.151245, -4820, 900)
                    player:Freeze(true)
                    ...
                    timer.Simple(0.85, function()
                        if IsValid(player) and IsValid(traceEnt) then
                            ...
                            traceEnt:SetPos(tracePos)

                            timer.Create('Kill_Player_'..traceEnt:SteamID64(), 10, 1, function()
                                if IsValid(traceEnt) and traceEnt:GetPos():WithinAABox( Vector(-1358.482056, -4554.435547, 573.549988), Vector(-1912.463013, -5194.814941, 1294.059937) ) then
                                    traceEnt:SetHealth(traceEnt:Health()-10)
                                    traceEnt:SetNWBool('KillZonePlayer', true)

                                    if timer.RepsLeft( 'Kill_Player_'..traceEnt:SteamID64() ) <= 1 then
                                        traceEnt:Kill()
                                    end
                                end
                            end)
    ...
    if SERVER then
        ...
        if IsValid(player) and CurTime() - self.KamuiSelfStart >= self.KamuiSelfCD and not self.IsAttacking and not self.SusanooActive then
            ...
            timer.Simple(1, function()
                if IsValid(self) and IsValid(player) then
                    if player:GetPos():WithinAABox( Vector(-1358.482056, -4554.435547, 573.549988), Vector(-1912.463013, -5194.814941, 1294.059937) ) and player:GetNWVector('expos') != nil then
                        player:SetPos(player:GetNWVector('expos'))
                        player:SetNWVector('expos', nil)
                    else
                        player:SetNWVector('expos', player:GetPos())
