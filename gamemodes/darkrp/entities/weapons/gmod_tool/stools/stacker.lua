...
if (CLIENT) then
	...
	language.Add("Tool.stacker.desc", "Позволяет стакать пропы")

	language.Add("Tool.stacker.left",    "По объекту для создания стака")

	language.Add("Undone_stacker",    "Отмена стакнутого(ых) пропа(ов)")
...
CreateConVar("stacker_delay",             0.7, bit.bor(FCVAR_NOTIFY, FCVAR_REPLICATED, FCVAR_SERVER_CAN_EXECUTE)) -- determines the amount of time that must pass before a player can use stacker again
...
function TOOL:LeftClick(trace)
	...
	if (self:GetOwner().LastStackTime and self:GetOwner().LastStackTime + self:GetDelay() > CurTime() ) then return false end
...
function TOOL.BuildCPanel(cpanel)
	...
	local rlabel = vgui.Create( "DLabel", cpanel )

	rlabel:SetText( 'Отношение:' )

	rlabel:SetColor(Color(0,0,0))

	local rcombo = vgui.Create( "DComboBox", cpanel  )

	rcombo:SetConVar( 'stacker_mode')

	rcombo:SetSize(200,25)

	rcombo:AddChoice('Мир', 1)

	rcombo:AddChoice('Проп', 2)

	rcombo:SetPos(100,0)

	function rcombo:OnSelect( index, value, data )

		if (stacker_mode) then return end

		RunConsoleCommand( 'stacker_mode', tostring( data or value ) )

	end

	cpanel:AddItem( rlabel, rcombo )



	cpanel:NumSlider('Количество:', "stacker_count", 1, GetConVarNumber("stacker_max_count"),0)



	local slabel = vgui.Create( "DLabel", cpanel )

	slabel:SetText( 'Направление:' )

	slabel:SetColor(Color(0,0,0))

	local scombo = vgui.Create( "DComboBox", cpanel  )

	scombo:SetConVar('stacker_dir')

	scombo:SetSize(200,25)

	scombo:AddChoice('Вверх', DIRECTION_UP)

	scombo:AddChoice('Вниз', DIRECTION_DOWN )

	scombo:AddChoice('Вперед', DIRECTION_FRONT)

	scombo:AddChoice('Назад', DIRECTION_BEHIND)

	scombo:AddChoice('Вправо', DIRECTION_RIGHT)

	scombo:AddChoice('Влево', DIRECTION_LEFT)

	scombo:SetPos(100,0)

	function scombo:OnSelect( index, value, data )

		if (stacker_dir) then return end

		RunConsoleCommand( 'stacker_dir', tostring( data or value ) )

	end

	cpanel:AddItem( slabel, scombo )



	cpanel:Button('Сброк настроек',"stacker_resetoffsets", 'reset')



	cpanel:NumSlider('Смещение по X',"stacker_offsetx", - GetConVarNumber("stacker_max_offsetx"), GetConVarNumber("stacker_max_offsetx"), 2 )

	cpanel:NumSlider('Смещение по Y',"stacker_offsety", - GetConVarNumber("stacker_max_offsety"), GetConVarNumber("stacker_max_offsety"), 2 )

	cpanel:NumSlider('Смещение по Z',"stacker_offsetz", - GetConVarNumber("stacker_max_offsetz"), GetConVarNumber("stacker_max_offsetz"), 2 )



	cpanel:NumSlider('Наклон по оси X',"stacker_rotp", -360, 360, 2 )

	cpanel:NumSlider('Наклон по оси Y',"stacker_rotr", -360, 360, 2 )

	cpanel:NumSlider('Наклон по оси Z',"stacker_roty", -360, 360, 2 )

	

	cpanel:CheckBox("Поворот относительно количества стаков","stacker_recalc" )

	cpanel:CheckBox("Применить материал","stacker_material" )

	cpanel:CheckBox("Применить цвет","stacker_color" )

	cpanel:CheckBox("Показывать все пропы перед стаком","stacker_ghostall" )
