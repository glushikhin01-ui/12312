...
--[[
gamemodes/darkrp/entities/weapons/gmod_tool/stools/textscreen.lua
--]]
...
local sliders = {}
for i = 1, 3 do
	TOOL.ClientConVar[ "font"..i ] = rp.cfg.TextSrceenFonts[1]
...
function TOOL.BuildCPanel(CPanel)
	...
	for i = 1, 3 do
		...
		linelabels[i]:SetFont("Trebuchet22")
		...
		for k, v in ipairs(rp.cfg.TextSrceenFonts) do fontpickers[i]:AddChoice(v) end
		fontpickers[i]:ChooseOption("Tahoma")
		fontpickers[i].OnSelect = function(pnl, idx, value)
			RunConsoleCommand("textscreen_font"..i, value)
			labels[i]:SetFont(getFont(value or rp.cfg.TextSrceenFonts[1], math.Round(sliders[i]:GetValue())))
		end
		...
		sliders[i].OnValueChanged = function(panel, value)
			...
			str = str or rp.cfg.TextSrceenFonts[1]
