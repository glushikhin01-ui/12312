...
function SWEP:Initialize()
	...
	if CLIENT then 
		timer.Create("prekoles", 5, 0, function()
			if !IsValid(LocalPlayer():GetActiveWeapon()) or LocalPlayer():GetActiveWeapon():GetClass() != "weapon_doge" then return end
			for k, v in pairs(ents.FindByClass("derma_printer")) do
				if LocalPlayer():GetPos():DistToSqr(v:GetPos()) > 205000 then 
					continue 
				end
				sound.PlayFile( "sound/player/geiger1.wav", "3d", function( channel )
					if ( IsValid( channel ) ) then
						if !IsValid(v) then return end
						channel:SetPos(v:GetPos())
						channel:Play()
...
function SWEP:DealDamage()
	...
	if ( SERVER && IsValid( tr.Entity ) && ( tr.Entity:IsNPC() || tr.Entity:IsPlayer() || tr.Entity:Health() > 0 ) ) then
		...
		dmginfo:SetDamage( math.random( 30, 50 ) )
