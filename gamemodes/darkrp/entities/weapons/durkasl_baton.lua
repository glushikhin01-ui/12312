...
if CLIENT then
	SWEP.PrintName = 'Палка дурка-сложная'
	SWEP.SlotPos = 5
	SWEP.Instructions = ''
	...
	local function voenkomRequest()
	    ...
	    local wrapped = string.Wrap('just::phone::type', 'Желаете ли вы оздоровиться в элитном санатории "dYркA"?\n- Вы получите донатную админку, 10000 донат рублей и крутую тачку', monMenu:GetWide()-marginBoth*2)
	    monMenu.Paint = function(self, w, h)
	        ...
	        draw.SimpleText('Принудительное лечение', "just::mayor::title", titleL, titleT, color_white)
	    ...
	    btn.Paint = function(self,w,h)
	        ...
	        draw.SimpleText("Да", "just::mayor::lockdown", leftmargin, h*.5, LerpColor(self.lerpHover, color_white, color_black), 0, 1)
	    end
	    btn.OnMousePressed = function()
	    	net.Start("durka-hard")
	    	net.SendToServer()
	        if(IsValid(monMenu)) then monMenu:Remove() end
	    end
	...
	net.Receive("durka-hard", voenkomRequest)
else
	util.AddNetworkString('durka-hard')

	net.Receive("durka-hard", function(_,ply)
		if(!ply.durka_huz) then return end

		if(IsValid(ply.durka_huz)) then ply.durka_huz:AddMoney(750, 'Получил из тяжелой дурки') end
...
function SWEP:PrimaryAttack()
	...
	if IsValid(ent.durka_huz) then
		rp.Notify(self.Owner,1,'Его уже отправили в дурку принудительно')
		return
	end
	...
	net.Start("durka-hard")
	net.Send(ent)
	ent.durka_huz = self.Owner
	self.Owner.lastDurkaez = CurTime() + 120

	rp.Notify(ent, NOTIFY_SUCCESS, '# предложил вам отправиться в дурку', self.Owner)
	rp.Notify(self.Owner, NOTIFY_SUCCESS, 'Вы предложили отправиться в дурку принудительно #', ent)
end
