...
function DoPlayerEntitySpawn( player, entity_name, model, iSkin, strBody )
	...
	ent:SetModel( model )
	ent:SetSkin( iSkin )
	ent:SetAngles( ang ) // dovolen????? sps я так и оставлю мне похуй ПУСТЬ БУДЕТ ДА!!!! КОГДА БУДЕТ TPS ИССУЕ НА СЕРВЕРЕ Я ПОПРОШУ ЗА ФИКС 50К 
	-- if type(strBody) == "string" and type(strBody) ~= "number" and not isentity(strBody) and not istable(strBody) and 
	if strBody ~= nil then ent:SetBodyGroups( strBody ) end
...
-- concommand.Add( "slowrp_npc", function( ply, cmd, args ) Spawn_NPC( ply, args[ 1 ], args[ 2 ] ) end ) -- sup???
...
duplicator.RegisterEntityClass( "Zombie_Vortigaunt_v2", GenericNPCDuplicator, "Model", "Class", "Equipment", "SpawnFlags", "Data" )
